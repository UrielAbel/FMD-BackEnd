...
router.get('/confirmEmail/:token', confirmEmailValidator, async (req, res, next) => {
    ...
    try {
        ...
        const tokenOwner = await Users.findOne({ email: email })
        ...
        if (receivedToken === tokenDB) {
            ...
            try {
                const confirmedUser = await Users.findOneAndUpdate({ email: email }, { $set: { emailConfirmed: true } }, { new: true })

                const deleteTokenDB = await Users.findOneAndUpdate({ email: email }, { $unset: { verificationToken: 1 } }, { new: true })
